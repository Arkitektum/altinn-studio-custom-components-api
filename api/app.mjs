 ...
 *
 * Kept apart from starting the process so the endpoints can be exercised without binding a port: the routing, the
 * CORS restriction, and the shape every handler shares. Building it here also means the caches below are created
 * once per app rather than once per process, so a test can have a clean one.
 *
